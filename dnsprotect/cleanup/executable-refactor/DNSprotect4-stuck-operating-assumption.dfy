...
module LucidProg refines LucidBase { 
   ...
   class Globals ... {
      ...
      ghost predicate stateInvariant2 (time: nat, timestamp: uint8, lastTime : nat)
      reads this
      {  (  timestampOn.val == timeOn % Sys.T  )
      && (  actualTimeOn <= timeOn  )
      && (  timeOn <= time  )
      ...
      // Operating assumptions that depend on system state
      ...
      // validInterArrival and stateInvariantPreservation is, I think, for 
      // a generic multi-packet / flow interpreter.
      // interarrival time constraints that are independent of system state.
      // All we should _really_ need here is that 
      // time is monotonically increasing and a density property is preserved.
      ...
      ) 
      {
            ...
            (time <= nextTime)
         // && ((time % Sys.T) == timestamp)
         // && ((nextTime % Sys.T) == nextTimestamp)
         && (nextEvent.ProcessPacket? || nextEvent.SetFiltering?)
         // Stuck here. 
         // If the next event is SetFiltering... 
         // protecting may not equal protectingImpl
         && nextTime - time < Sys.T - Globals.I
      }

      lemma stateInvariantPreservationTest(
               lastTime:nat,
               time:nat,
               timestamp:uint8,
               nextEvent:Event, 
      ...
      { 
         if nextEvent.SetFiltering? {

            if (filtering.val) {
               // assume{:axiom} (nextTime < actualTimeOn + Sys.T);
               // the time of the event is no more than actualTimeOn + Sys.T
               // operatingAssumptions makes this true for packet events
               // why is it true for setFiltering events? 
               // its not true. 
               // say the program is initialized (actualTimeOn == 0)
               // then after time T, filtering turns on. 
               // then we have a setFiltering event at time T + x, where x > 0, 
               // so T + x < T + x is FALSE.
               // LEFT OFF HERE
               // okay then... not sure what to do here...
               // so then (nextTimeStamp - timestampOn.val) % T >= Q doesn't
               // necessarily have to be true for setFiltering events? 
               // protectImplmnt (sys.timestamp) is only used in the call to filter
               // this is only used in the ProcessPacket handler. 
               // So we don't care about protectImplmnt for SetFiltering events. 
               // What to do with that information though...
               assume {:axiom} (
                  (nextTime - actualTimeOn) >= Q as nat
                  ==>
                  (nextTimestamp - timestampOn.val) % Sys.T >= Q
               );
         ...
         else if nextEvent.ProcessPacket? {
            if (filtering.val) {
               assume{:axiom} (filtering.val ==> nextTime < actualTimeOn + Sys.T);
               assert (
                  filtering.val ==> (
                     (nextTime - actualTimeOn) >= Q as nat
                     ==>
      ...
      ghost predicate singleQueueConstraints(startTime : nat, startTimestamp : uint8, queue1 : seq<TimedEvent>)
      {
         |queue1| > 0 ==> (
            // requirement for the first event
            validArrivalTime(startTime, startTimestamp, queue1[0].event, queue1[0].time, queue1[0].timestamp)
            // requirements for all subsequent events
            && (forall i :: 0 <= i < (|queue1|-1) ==> 
               validArrivalTime(queue1[i].time, queue1[i].timestamp, queue1[i+1].event, queue1[i+1].time, queue1[i+1].timestamp))
            && (forall i :: 0 <= i < |queue1| ==> validTimedEvent(queue1[i])) // trivial: time % T == timestamp
         )
      }

      // function mergeQueues(startTime : nat, startTimestamp : uint8, q1 : seq<TimedEvent>, q2 : seq<TimedEvent>)  : seq<TimedEvent>
      //    // both queues must be legal to execute at the start time.
      //    requires  |q1| > 0 ==> singleQueueConstraints(startTime, startTimestamp, q1)
      //    requires  |q2| > 0 ==> singleQueueConstraints(startTime, startTimestamp, q2)
      // {
   ...
   method test() {
      ...
      var te := TimedEvent(ProcessPacket(false, 1), 1, 1);
      var _ := runTimedEvent(prog, te); 
   }

...
abstract module LucidBase {
   ...
   method runTimedEvent (prog : Program, te : TimedEvent) 
      ...
      ensures  prog.sys.lastTime == old(prog.sys.time)
   {
      ...
      // for now, ignore generated events
   ...
   method runTimedEventAndRecirc(prog : Program, te : TimedEvent)
      // run a single timed event and then immediately run the generated events. 
      // Drop the events generated by generated events
      modifies prog.sys
      modifies prog.globals
      modifies prog`generatedEvents
      requires prog.generatedEvents == {}
      ...
      ensures  prog.sys.lastTime == old(prog.sys.time)
   {
      ...
      var generatedEvents := prog.generatedEvents;
      prog.generatedEvents := {};

      // run recirculation events immediately, in same clock cycle
      while |generatedEvents| > 0
      invariant prog.globals.stateInvariant(prog.sys.time, prog.sys.timestamp, prog.sys.lastTime)
      invariant prog.sys.time == te.time
      invariant prog.sys.timestamp == te.timestamp
      invariant prog.sys.lastTime == old(prog.sys.time)
      invariant prog.generatedEvents == {}
      {
         var generatedEvent :| generatedEvent in generatedEvents;
         if (generatedEvent.ports == {}) { // recirc event, process
            var recircEvent := 
                   TimedEvent(generatedEvent.event, prog.sys.time, prog.sys.timestamp);
            assume {:axiom} prog.globals.operatingAssumptions(generatedEvent.event, prog.sys.time, prog.sys.timestamp, prog.sys.lastTime);
            prog.dispatch(recircEvent.event); 
            // drop events generated by recirculation events... (!!!)
            prog.generatedEvents := {};
         }
         ...
         //    modifies prog`generatedEvents
         //    modifies this`queue
         //    invariant |this.queue| > 0
         //    invariant validQueue(this.queue)
         //    invariant prog.globals.stateInvariant (prog.sys.time, q[0].timestamp, prog.sys.lastTime)
         // {
         //    assert |this.queue| > 0;
         ...
         //       this.queue := this.queue + [recircEvent];
         //    }
         //    prog.generatedEvents := prog.generatedEvents - {generatedEvent};
         // }
         // assert prog.globals.stateInvariant (prog.sys.time, q[0].timestamp, prog.sys.lastTime);
